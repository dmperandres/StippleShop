...
void  _gl_widget::read_image(string File_name)
{
  ...
  if (Width_aux!=Original_image->cols || Height_aux!=Original_image->rows){
      ...
      MsgBox.exec();
    cv::resize(*Original_image.get(),*Original_image.get(),cv::Size(Width_aux,Height_aux),0,0,CV_INTER_CUBIC);
  }
  ...
  if (Original_image->channels()==1){
    ...
    *Image_cv_gray.get()=Original_image->clone();
    ...
    cvtColor(*Image_cv_gray.get(),*Image_cv_color.get(),CV_GRAY2RGB,3);
    // The input image is copied to the frame-buffer as an initial value
    cvtColor(*Original_image.get(),*Frame_buffer.get(),CV_GRAY2BGR,3);
  }
  else{
    // Color
    Image_cv_color->release();
    *Image_cv_color.get()=Original_image->clone();
    ...
    cvtColor(*Image_cv_color.get(),*Image_cv_gray.get(),CV_BGR2GRAY,1);
    // The input image is copied to the frame-buffer as an initial value
...
void _gl_widget::initializeGL()
{
  ...
  Program1=Shader.load_shaders("shaders/basic.vert","shaders/basic.frag");
  if (Program1==0){
    exit(-1);
  }
  // adjust the sizes
...
void _gl_widget::refresh_image()
{
  if (Image_loaded){
    if (Images.get_data(Selected_image_index)->channels()==1){
      cv::cvtColor(*Images.get_data(Selected_image_index).get(),*Frame_buffer.get(),CV_GRAY2BGR,3);
    }
    else{
...
{
  if (Image->channels()==1){
    cv::cvtColor(*Image,*Frame_buffer.get(),CV_GRAY2BGR,3);
  }
  else{
...
void _gl_widget::create_filters_from_blocks()
{
  ...
  for (unsigned int i=0;i<Vec_order->size();i++){
    ...
    #endif
    #ifdef DEFINE_FILTER_CONTRAST_AWARE_STIPPLING
    case FILTER_STIPPLING_CAS:Filters.set(Name,make_shared<_filter_stippling_sas_ui>(this,(*Map_filters_parameters)[Name],Name));break; // STIPPLING_CAS
    #endif
