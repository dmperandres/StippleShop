...
public:
  ...
  _map_list<std::shared_ptr<_filter>> Filters;
  // output images
  //_map_list<cv::Mat *> Images;
  _map_list<std::shared_ptr<cv::Mat>> Images;
  // Pipeline (maintains the order of filters)
  //vector<_filter *> Pipeline;
  std::vector<std::shared_ptr<_filter>> Pipeline;
  ...
  void create_filters_from_blocks(std::map<std::string, _block> *Map_data_blocks1, std::vector<std::string> *Vec_order1, std::map<std::string, std::map<std::string, std::string> > *Map_filters_parameters1);
...
private:
  ...
  std::vector<std::vector<float>> Scale_drawing_mode;
  std::vector<std::vector<_vertex2f>> Translation;
  ...
  std::shared_ptr<cv::Mat> Image_cv_gray;
  std::shared_ptr<cv::Mat> Image_cv_gray1;
  std::shared_ptr<cv::Mat> Image_cv_color;
  std::shared_ptr<cv::Mat> Frame_buffer;
  std::shared_ptr<cv::Mat> Original_image;
