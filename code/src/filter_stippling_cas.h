...
#ifndef _FILTER_STIPPLING_CA_H
#define _FILTER_STIPPLING_CA_H
...
#define DEFINED_FILTER_CONTRAST_AWARE_STIPPLING

namespace _f_stippling_cas_ns
{
  // Kernel size
  // parameter 1
  ...
  const float HALFTONING_CAH_EXPONENT_DEFAULT=2.0;
  const int HALFTONING_CAH_G_PLUS_DEFAULT=5;
  const int HALFTONING_CAH_G_MINUS_DEFAULT=5;
  const float HALFTONING_CAH_K_DEFAULT=0;
  const float HALFTONING_CAH_STIPPLE_SIZE_MIN=1;
  const float HALFTONING_CAH_STIPPLE_SIZE_MAX=2;
}

class _gl_widget;
class _qtw_filter_stippling_cas;
...
class _filter_stippling_cas : public _filter
{
public:
    _filter_stippling_cas();
    ~_filter_stippling_cas(){};
    void reset_data();
...
class _filter_stippling_cas_ui :public _filter_stippling_cas
{
public:
  _filter_stippling_cas_ui(_gl_widget *GL_widget1,std::string Name1="Contrast-Aware Stippling parameters");
  _filter_stippling_cas_ui(_gl_widget *GL_widget1,std::map<std::string,std::string> &Parameters,std::string Box_name="Contrast-Aware Stippling parameters");
  ~_filter_stippling_cas_ui();
  void reset_data();
  ...
  void parameter1(int Value){_filter_stippling_cas::kernel_size(Value);};
  int parameter1(){return _filter_stippling_cas::kernel_size();};

  void parameter2(float Value){_filter_stippling_cas::exponent(Value);};
  float parameter2(){return _filter_stippling_cas::exponent();};

  void parameter3(int Value){_filter_stippling_cas::g_plus(Value);};
  int parameter3(){return _filter_stippling_cas::g_plus();};

  void parameter4(int Value){_filter_stippling_cas::g_minus(Value);};
  int parameter4(){return _filter_stippling_cas::g_minus();};

  void parameter5(float Value){_filter_stippling_cas::k(Value);};
  float parameter5(){return _filter_stippling_cas::k();};

  private:
  _qtw_filter_stippling_cas *Qtw_filter_stippling_cas;
};
...
class _qtw_filter_stippling_cas: public QWidget
{
  Q_OBJECT
public:

  _qtw_filter_stippling_cas(_gl_widget *GL_widget1,_filter_stippling_cas_ui *Filter1,std::string Box_name="Contrast-Aware Stippling parameters");
...
private:
  ...
  _filter_stippling_cas_ui *Filter;
