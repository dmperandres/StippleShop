...
void _filter_dog::update()
{
  ...
  if (Output_image_0->channels()!=Num_channels_output_image_0){// Different number of channels
    if (Output_image_0->channels()==3){
      // conversion
      cvtColor(Borders_dog_big,*Output_image_0,cv::COLOR_GRAY2RGB,3);
    }
...
_filter_dog_ui::_filter_dog_ui(_gl_widget *GL_widget1,std::string Name1)
{
  Name=Name1;
...
_filter_dog_ui::_filter_dog_ui(_gl_widget *GL_widget1, std::map<std::string, std::string> &Parameters, std::string Name1)
{
...
void _filter_dog_ui::write_parameters(std::map<std::string, std::string> &Parameters)
{
  char Aux[100];

  sprintf(Aux,"%d",parameter1());
  Parameters["umbral"]=std::string(Aux);
  sprintf(Aux,"%d",parameter2());
  Parameters["kernel_size_big"]=std::string(Aux);
  sprintf(Aux,"%d",parameter3());
  Parameters["kernel_size_small"]=std::string(Aux);
}
...
_qtw_filter_dog::_qtw_filter_dog(_gl_widget *GL_widget1,_filter_dog_ui *Filter1,std::string Box_name)
{
  QString Str;
