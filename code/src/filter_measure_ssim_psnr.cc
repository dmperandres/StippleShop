...
void _filter_measure_ssim_psnr::update()
{
  ...
  if (Input_image_1->channels()!=Num_channels_input_image_1){// Different number of channels
    if (Input_image_1->channels()==3){
      ...
      cvtColor(*Input_image_1,*Aux_input_image1,cv::COLOR_BGR2GRAY,1);
    }
    else std::cout << "Error in the number of channels in the input image " << __LINE__ << " " << __FILE__ << std::endl;
  }
  else{// the same number of channels
  ...
  if (Output_image_0->channels()!=Num_channels_output_image_0){// Different number of channels
    if (Output_image_0->channels()==3){
      ...
      cvtColor(*Aux_input_image0,*Output_image_0,cv::COLOR_GRAY2RGB,3);
    }
...
_filter_measure_ssim_psnr_ui::_filter_measure_ssim_psnr_ui(_gl_widget *GL_widget1,std::string Name1)
{
  Name=Name1;
...
_filter_measure_ssim_psnr_ui::_filter_measure_ssim_psnr_ui(_gl_widget *GL_widget1,std::map<std::string,std::string> &Parameters,std::string Box_name)
{
...
_qtw_filter_measure_ssim_psnr::_qtw_filter_measure_ssim_psnr(_gl_widget *GL_widget1,_filter_measure_ssim_psnr_ui *Filter1,std::string Box_name)
{
  QString Str;
