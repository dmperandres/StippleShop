...
using namespace _f_stippling_sas_ns;
...
_filter_stippling_sas::_filter_stippling_sas()
{
  Kernel_size=SAS_KERNEL_SIZE_DEFAULT;
  Exponent=SAS_EXPONENT_DEFAULT;
  G_plus=SAS_G_PLUS_DEFAULT;
  G_minus=SAS_G_MINUS_DEFAULT;
  K=SAS_K_DEFAULT;

  Num_channels_input_image_0=1;
  Num_channels_output_image_0=1;

  Type_filter=_f_filter_ns::FILTER_STIPPLING_SAS;
...
void _filter_stippling_sas::reset_data()
{
  Kernel_size=SAS_KERNEL_SIZE_DEFAULT;
  Exponent=SAS_EXPONENT_DEFAULT;
  G_plus=SAS_G_PLUS_DEFAULT;
...
void _filter_stippling_sas::stippling(cv::Mat *Input_image0,cv::Mat *Output_image0)
{
  int Value,Error;
  ...
  for (int Row=Kernel_radius;Row<Input_image_aux.rows-Kernel_radius;Row++){
    for (int Col=Kernel_radius;Col<Input_image_aux.cols-Kernel_radius; Col++){
      ...
      Stipple_size=SAS_STIPPLE_SIZE_MIN+((SAS_STIPPLE_SIZE_MAX-SAS_STIPPLE_SIZE_MIN)*(float)(255-Value)/255.);
...
void _filter_stippling_sas::update()
{
...
_filter_stippling_sas_ui::_filter_stippling_sas_ui(_gl_widget *GL_widget1,string Name1)
{
  Name=Name1;
  Qtw_filter_stippling_sas=new _qtw_filter_stippling_sas(GL_widget1,this,Name);
}
...
_filter_stippling_sas_ui::_filter_stippling_sas_ui(_gl_widget *GL_widget1, std::map<string, string> &Parameters, string Box_name)
{
  read_parameters(Parameters);
  Name=Box_name;
  Qtw_filter_stippling_sas=new _qtw_filter_stippling_sas(GL_widget1,this,Box_name);
}
...
_filter_stippling_sas_ui::~_filter_stippling_sas_ui()
{
  delete Qtw_filter_stippling_sas;
}
...
void _filter_stippling_sas_ui::reset_data()
{
  _filter_stippling_sas::reset_data();
  Qtw_filter_stippling_sas->set_parameter1(parameter1());
  Qtw_filter_stippling_sas->set_parameter2(parameter2());
  Qtw_filter_stippling_sas->set_parameter3(parameter3());
  Qtw_filter_stippling_sas->set_parameter4(parameter4());
  Qtw_filter_stippling_sas->set_parameter5(parameter5());
  hide();
}
...
void _filter_stippling_sas_ui::show()
{
  Qtw_filter_stippling_sas->show();
}
...
void _filter_stippling_sas_ui::hide()
{
  Qtw_filter_stippling_sas->hide();
}
...
void *_filter_stippling_sas_ui::get_link()
{
  return Qtw_filter_stippling_sas->get_link();
}
...
void _filter_stippling_sas_ui::read_parameters(std::map<std::string,std::string> &Parameters)
{
  if (Parameters["_INI_"]=="EDITOR"){// default parameters
    parameter1(SAS_KERNEL_SIZE_DEFAULT);
    parameter2(SAS_EXPONENT_DEFAULT);
    parameter3(SAS_G_PLUS_DEFAULT);
    parameter4(SAS_G_MINUS_DEFAULT);
    parameter5(SAS_K_DEFAULT);
  }
  else{// Parameters from file or from initialised filter
    try{
      if (Parameters["kernel_size"]=="default") parameter1(SAS_KERNEL_SIZE_DEFAULT);
      else parameter1(atoi(Parameters["kernel_size"].c_str()));
      if (Parameters["exponent"]=="default") parameter2(SAS_EXPONENT_DEFAULT);
      else parameter2(atof(Parameters["exponent"].c_str()));
      if (Parameters["g+"]=="default") parameter3(SAS_G_PLUS_DEFAULT);
      else parameter3(atoi(Parameters["g+"].c_str()));
      if (Parameters["g-"]=="default") parameter4(SAS_G_MINUS_DEFAULT);
      else parameter4(atoi(Parameters["g-"].c_str()));
      if (Parameters["k"]=="default") parameter5(SAS_K_DEFAULT);
...
void _filter_stippling_sas_ui::write_parameters(std::map<std::string,std::string> &Parameters)
{
  char Aux[100];
...
_qtw_filter_stippling_sas::_qtw_filter_stippling_sas(_gl_widget *GL_widget1,_filter_stippling_sas_ui *Filter1,string Box_name)
{
  QString Str;
  char Aux[100];
...
void _qtw_filter_stippling_sas::set_parameter1(int Value)
{
  QString Str;
...
void _qtw_filter_stippling_sas::set_parameter2(float Value)
{
  QString Str;
  char Aux[100];
...
void _qtw_filter_stippling_sas::set_parameter3(int Value)
{
  QString Str;
...
void _qtw_filter_stippling_sas::set_parameter4(int Value)
{
  QString Str;
...
void _qtw_filter_stippling_sas::set_parameter5(float Value)
{
  QString Str;
  char Aux[100];
...
void _qtw_filter_stippling_sas::set_parameter1_slot(int Size)
{
  QString Str;
  char Aux[100];
...
void _qtw_filter_stippling_sas::set_parameter2_slot(int Size)
{
  QString Str;
  char Aux[100];
...
void _qtw_filter_stippling_sas::set_parameter3_slot(int Size)
{
  QString Str;
  char Aux[100];
...
void _qtw_filter_stippling_sas::set_parameter4_slot(int Size)
{
  QString Str;
  char Aux[100];
...
void _qtw_filter_stippling_sas::set_parameter5_slot(int Size)
{
  QString Str;
  char Aux[100];
