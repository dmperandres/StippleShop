...
_filter_threshold::_filter_threshold()
{
  Threshold_type=THRESHOLD_TYPE_DEFAULT;
  Threshold=THRESHOLD_THRESHOLD_DEFAULT;
  Max_binary_value=THRESHOLD_MAX_BINARY_VALUE_DEFAULT;

  Num_channels_input_image_0=1;
  Num_channels_output_image_0=1;
  Num_channels_input_image_1=1;

  Type_filter=FILTER_THRESHOLD;
...
void _filter_threshold::reset_data()
{
  Threshold_type=THRESHOLD_TYPE_DEFAULT;
  Threshold=THRESHOLD_THRESHOLD_DEFAULT;
  Max_binary_value=THRESHOLD_MAX_BINARY_VALUE_DEFAULT;
...
void _filter_threshold::update()
{
  ...
  if (Output_image_0->channels()!=Num_channels_output_image_0){// Different number of channels
    if (Output_image_0->channels()==3){
      ...
      cv::threshold(*Aux_input_image,*Aux_output_image,Threshold,Max_binary_value,Threshold_type);
    ...
    cv::threshold(*Aux_input_image,*Output_image_0,Threshold,Max_binary_value,Threshold_type);
  }
...
_filter_threshold_ui::_filter_threshold_ui(_gl_widget *GL_widget1,string Name1)
{
  Name=Name1;
  Qtw_filter_threshold=new _qtw_filter_threshold(GL_widget1,this,Name);
}
...
_filter_threshold_ui::_filter_threshold_ui(_gl_widget *GL_widget1, std::map<string, string> &Parameters, string Box_name)
{
  read_parameters(Parameters);
  Name=Box_name;
  Qtw_filter_threshold=new _qtw_filter_threshold(GL_widget1,this,Box_name);
}
...
_filter_threshold_ui::~_filter_threshold_ui()
{
  delete Qtw_filter_threshold;
}
...
void _filter_threshold_ui::reset_data()
{
  _filter_threshold::reset_data();
  Qtw_filter_threshold->set_parameter1(parameter1());
  hide();
}
...
void _filter_threshold_ui::show()
{
  Qtw_filter_threshold->show();
}
...
void _filter_threshold_ui::hide()
{
  Qtw_filter_threshold->hide();
}
...
void *_filter_threshold_ui::get_link()
{
  return Qtw_filter_threshold->get_link();
}
...
void _filter_threshold_ui::read_parameters(std::map<std::string,std::string> &Parameters)
{
  if (Parameters["_INI_"]=="EDITOR"){// default parameters
    parameter1(THRESHOLD_TYPE_DEFAULT);
    parameter2(THRESHOLD_THRESHOLD_DEFAULT);
    parameter3(THRESHOLD_MAX_BINARY_VALUE_DEFAULT);
  }
  else{// Parameters from file or from initialised filter
    try{
      if (Parameters["type"]=="default") parameter1(THRESHOLD_TYPE_DEFAULT);
      else parameter1(Operations_inv[Parameters["type"]]);

      if (Parameters["threshold"]=="default") parameter2(THRESHOLD_THRESHOLD_DEFAULT);
      else parameter2(atoi(Parameters["threshold"].c_str()));

      if (Parameters["max_binary_value"]=="default") parameter3(THRESHOLD_MAX_BINARY_VALUE_DEFAULT);
      else parameter3(atoi(Parameters["max_binary_value"].c_str()));
    }
...
void _filter_threshold_ui::write_parameters(std::map<std::string,std::string> &Parameters)
{
  char Aux[100];

  Parameters["type"]=Operations[(_threshold_type)parameter1()];
  sprintf(Aux,"%d",parameter2());
  Parameters["threshold"]=string(Aux);
  sprintf(Aux,"%d",parameter3());
  Parameters["max_binary_value"]=string(Aux);
}
...
_qtw_filter_threshold::_qtw_filter_threshold(_gl_widget *GL_widget1,_filter_threshold_ui *Filter1,string Box_name)
{
  QString Str;
  ...
  for (unsigned int i=0;i<(unsigned int) THRESHOLD_LAST;i++){
...
void _qtw_filter_threshold::set_parameter1(int Value)
{
...
void _qtw_filter_threshold::set_parameter2(int Value)
{
  QString Str;
...
void _qtw_filter_threshold::set_parameter3(int Value)
{
  QString Str;
...
void _qtw_filter_threshold::set_parameter1_slot(int Value)
{
  Filter->parameter1((_threshold_type) Value);
...
void _qtw_filter_threshold::set_parameter2_slot(int Size)
{
  QString Str;
  char Aux[100];
...
void _qtw_filter_threshold::set_parameter3_slot(int Size)
{
  QString Str;
  char Aux[100];
