...
: QOpenGLWidget(Parent)
{
  ...
  Image_cv_gray=std::make_shared<cv::Mat>();
  Image_cv_gray1=std::make_shared<cv::Mat>();
  Image_cv_color=std::make_shared<cv::Mat>();
  Frame_buffer=std::make_shared<cv::Mat>();
  Original_image=std::make_shared<cv::Mat>();
...
void  _gl_widget::read_image(std::string File_name)
{
...
void  _gl_widget::save_image(std::string File_name)
{
  _image_IO Image_io;
  cv::Mat Aux_image;
...
void _gl_widget::read_dots()
{
  ...
  // check that there is a dot folder
  QDir Test(Directory.currentPath()+"/dots");
  if(!Test.exists()) {
    QMessageBox::critical(this,"Error","There is not <b>dots</b> folder. Abort.");
    exit(-1);
  }
...
//  printf("Reading dots files\n");
  for (unsigned int i=_f_gl_widget_ns::RESOLUTION_300PPI;i<_f_gl_widget_ns::LAST_RESOLUTION;i++){
    for (unsigned int j=_f_gl_widget_ns::MIN_DOT_SIZE;j<=_f_gl_widget_ns::MAX_DOT_SIZE;j++){
      ...
      std::string File_name_aux=File_name+std::string(Name);
      Image_io.read_image(File_name_aux,*Dots[i][j]);
      cv::flip(*Dots[i][j],*Dots[i][j],0);
      if (Dots[i][j]->channels()!=1){
        std::cout << "Error: number of channels incorrect in dots files" << std::endl;
        exit(-1);
      }
    }
  }
...
void _gl_widget::initializeGL()
{
  ...
  #include "src/shaders/basic.vert"
  };

  static const GLchar * P0_fs[]={
  #include "src/shaders/basic.frag"
  };
...
void _gl_widget::set_image(std::string Image_name)
{
...
void _gl_widget::set_filter(std::string Filter_name)
{
...
void _gl_widget::update_aux(std::string Name)
{
...
void _gl_widget::update_effect(std::string Name)
{
...
void _gl_widget::read_data_effect(std::string Name)
{
...
void _gl_widget::write_data_effect(std::string Name)
{
  QJsonObject Filter_object;
...
void _gl_widget::read_effect(std::string Name)
{
...
void _gl_widget::create_filters_from_blocks()
{
  ...
  for (unsigned int i=0;i<Vec_order->size();i++){
    ...
    case FILTER_BILATERAL:Filters.set(Name,std::make_shared<_filter_bilateral_ui>(this,(*Map_filters_parameters)[Name],Name));break; // Bilateral
    #endif

    #ifdef DEFINE_FILTER_CANNY
    case FILTER_CANNY:Filters.set(Name,std::make_shared<_filter_canny_ui>(this,(*Map_filters_parameters)[Name],Name));break; // CANNY
    #endif

    #ifdef DEFINE_FILTER_CCVT
    case FILTER_CCVT:Filters.set(Name,std::make_shared<_filter_ccvt_ui>(this,(*Map_filters_parameters)[Name],Name));break; // BALZER'S CCVT
    #endif

    #ifdef DEFINE_FILTER_COMBINATION
    case FILTER_COMBINATION:{
        std::shared_ptr<_filter_combination_ui> Filter1=std::make_shared<_filter_combination_ui>(this,(*Map_filters_parameters)[Name],Name);
    ...
    case FILTER_CONTRAST_BRIGHT:Filters.set(Name,std::make_shared<_filter_contrast_bright_ui>(this,(*Map_filters_parameters)[Name],Name));break; // CONTRAST
    #endif

    #ifdef DEFINE_FILTER_DILATION
    case FILTER_DILATION:Filters.set(Name,std::make_shared<_filter_dilation_ui>(this,(*Map_filters_parameters)[Name],Name));break; // DILATION
    #endif

    #ifdef DEFINE_FILTER_DISTANCE_FIELD
    case FILTER_DISTANCE_FIELD:Filters.set(Name,std::make_shared<_filter_distance_field_ui>(this,(*Map_filters_parameters)[Name],Name));break; // FIELD DISTANCE
    #endif

    #ifdef DEFINE_FILTER_DOG
    case FILTER_DOG:Filters.set(Name,std::make_shared<_filter_dog_ui>(this,(*Map_filters_parameters)[Name],Name));break; // DOG
    #endif

    #ifdef DEFINE_FILTER_DOT_EBG
    case FILTER_DOT_EBG:
    {
      std::shared_ptr<_filter_dot_ebg_ui> Filter1=std::make_shared<_filter_dot_ebg_ui>(this,(*Map_filters_parameters)[Name],Name);
      Filters.set(Name,Filter1);
    ...
    case FILTER_DOT_SVG:
    {
      std::shared_ptr<_filter_dot_svg_ui> Filter1=std::make_shared<_filter_dot_svg_ui>(this,(*Map_filters_parameters)[Name],Name);
      Filters.set(Name,Filter1);
      Filter1->set_dots_images(&Dots);
    ...
    case FILTER_EROTION:Filters.set(Name,std::make_shared<_filter_erotion_ui>(this,(*Map_filters_parameters)[Name],Name));break; // DILATION
    #endif
    #ifdef DEFINE_FILTER_EXAMPLE_BASED_GRAYSCALE_STIPPLING
    case FILTER_STIPPLING_EBG:{
        std::shared_ptr<_filter_stippling_ebg> Filter1=std::make_shared<_filter_stippling_ebg_ui>(this,(*Map_filters_parameters)[Name],Name);
        Filters.set(Name,Filter1);
        Filter1->set_dots_images(&Dots);
    ...
    case FILTER_GAUSSIAN:Filters.set(Name,std::make_shared<_filter_gaussian_ui>(this,(*Map_filters_parameters)[Name],Name));break; // GAUSSIAN
    #endif

    #ifdef DEFINE_FILTER_HALFTONING_ADAPTIVE_CLUSTERING_SELECTIVE_PRECIPITATION
    case FILTER_HALFTONING_ACSP:Filters.set(Name,std::make_shared<_filter_halftoning_acsp_ui>(this,(*Map_filters_parameters)[Name],Name));break; // ADAPTIVE CLUSTERING SELECTIVE PRECIPITATION
    #endif

    #ifdef DEFINE_FILTER_HALFTONING_CONTRAST_AWARE
    case FILTER_HALFTONING_CAH:Filters.set(Name,std::make_shared<_filter_halftoning_cah_ui>(this,(*Map_filters_parameters)[Name],Name));break; // HALFTONING_CAH
    #endif

    #ifdef DEFINE_FILTER_HALFTONING_OSTROMOUKHOV
    case FILTER_HALFTONING_OST:Filters.set(Name,std::make_shared<_filter_halftoning_ost_ui>(this,(*Map_filters_parameters)[Name],Name));break; // HALFTONING
    #endif

    #ifdef DEFINE_FILTER_HALFTONING_SPACE_FILLING_CURVE
    case FILTER_HALFTONING_SFC:Filters.set(Name,std::make_shared<_filter_halftoning_sfc_ui>(this,(*Map_filters_parameters)[Name],Name));break; // SPACE FILLING CURVE
    #endif

    #ifdef DEFINE_FILTER_INVERSION
    case FILTER_INVERSION:Filters.set(Name,std::make_shared<_filter_inversion_ui>(this,(*Map_filters_parameters)[Name],Name));break; // INVERSION
    #endif

    #ifdef DEFINE_FILTER_KANG
    case FILTER_KANG:Filters.set(Name,std::make_shared<_filter_kang_ui>(this,(*Map_filters_parameters)[Name],Name));break; // KANG
    #endif

    #ifdef DEFINE_FILTER_MEASURE_SSIM_PSNR
    case FILTER_MEASURE_SSIM_PSNR:Filters.set(Name,std::make_shared<_filter_measure_ssim_psnr_ui>(this,Name));break; // MEASURE_SSIM_PSNR
    #endif

    #ifdef DEFINE_FILTER_RETINEX
    case FILTER_RETINEX:Filters.set(Name,std::make_shared<_filter_retinex_ui>(this,(*Map_filters_parameters)[Name],Name));break; // RETINEX
    #endif
    ...
    case FILTER_RWT:
      ...
      Filters.set(Name,std::make_shared<_filter_rwt_ui>(this,(*Map_filters_parameters)[Name],Name));
      break; // STIPPLING_RWT
    #endif
    ...
    case FILTER_STIPPLING_CAS:Filters.set(Name,std::make_shared<_filter_stippling_cas_ui>(this,(*Map_filters_parameters)[Name],Name));break; // STIPPLING_CAS
    #endif

    #ifdef DEFINE_FILTER_WCVD
    case FILTER_WCVD:
    {
      Filters.set(Name,std::make_shared<_filter_wcvd_ui>(this,(*Map_filters_parameters)[Name],Name));break; //WCVD
...
//      Filters.set(Name,Filter1);
    ...
    if (Filters.get_data(Name)->change_output_image_size()==false){ // not necessary to change
      Images.set(Name,std::make_shared<cv::Mat>());
...
void _gl_widget::create_filters_from_blocks(std::map<std::string,_block> *Map_data_blocks1, std::vector<std::string> *Vec_order1, std::map<std::string, std::map<std::string, std::string> > *Map_filters_parameters1)
{
...
void  _gl_widget::check2(std::string File_name1,std::string File_name2)
{
  Q_UNUSED(File_name1)
  Q_UNUSED(File_name2)
  ...
  Filters.set(Name,std::make_shared<_filter_measure_ssim_psnr_ui>(this,Filters_json_data[0],Name));
