...
void _filter_dot_svg::set_dots_images(vector<vector<cv::Mat *> > *Dots1)
{
  Dots=Dots1;
}
  ...
  if (Input_image_0->cols!=Input_image_1->cols || Input_image_0->rows!=Input_image_1->rows){
    ...
    if (Input_image_1->channels()!=Num_channels_input_image_1){// Different number of channels
      if (Input_image_1->channels()==3){
        ...
        cvtColor(*Input_image_0,*Aux_input_image1,CV_BGR2GRAY,1);
      }
      else cout << "Error in the number of channels in the input image " << __LINE__ << " " << __FILE__ << endl;
    }
    else{// the same number of channels
...
_filter_dot_svg_ui::_filter_dot_svg_ui(_gl_widget *GL_widget1, string Name1)
{
  Name=Name1;
...
_filter_dot_svg_ui::_filter_dot_svg_ui(_gl_widget *GL_widget1, std::map<string, string> &Parameters, string Box_name)
{
...
void _filter_dot_svg_ui::write_parameters(std::map<std::string,std::string> &Parameters)
{
  ...
  Parameters["dot_type"]=string(Aux);
  ...
  Parameters["antialias"]=string(Aux);

  sprintf(Aux,"%d",parameter4());
  Parameters["min_dot_size"]=string(Aux);

  sprintf(Aux,"%d",parameter5());
  Parameters["max_dot_size"]=string(Aux);
}
...
_qtw_filter_dot_svg::_qtw_filter_dot_svg(_gl_widget *GL_widget1,_filter_dot_svg_ui *Filter1,string Box_name)
{
  QString Str;
